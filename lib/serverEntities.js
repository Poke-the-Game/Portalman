...
ServerPlayer.prototype.isStable = function (x, y) {
  ...
  for (var i = 0 ; i < 4 /* positions.length */ ; i++) {
    ...
    if (collisionEntity && collisionEntity.owner && collisionEntity.owner.id === this.id) {
      ...
      side = collisionEntity.getSide(positions[i][0], positions[i][1])

      if (side === 'right' && typeof collisionEntity.portal.right !== 'boolean') {
        this.portalTo = collisionEntity.portal.right
      } else if (side === 'left' && typeof collisionEntity.portal.left !== 'boolean') {
        this.portalTo = collisionEntity.portal.left
      } else if (side === 'top' && typeof collisionEntity.portal.top !== 'boolean') {
        this.portalTo = collisionEntity.portal.top
      } else if (side === 'bottom' && typeof collisionEntity.portal.bottom !== 'boolean') {
...
ServerPlayer.prototype.updateTargetBlock = function () {
  ...
  for (var i = 0; i < line.length; i++) {
    if (colMap[line[i].x][line[i].y]) {
      ...
      var canPortal = isPortalalalalable(line[i].x, line[i].y)
      break
    }
  }

  if (targetBlock.id !== this.targetBlock.id) {
    this.changedSinceLastTick = true
  }

  this.targetBlock = {
    'id': targetBlock ? targetBlock.id : false,
    'canPortal': canPortal,
    'side': 'left' // TODO Actually determine what side we're looking at.
  }

  /*
  if (targetBlock) {
    console.log(targetBlock.pos)
  }
  */
}
