...
GameServer.prototype.worldUpdateEvent = function () {
  this.io.sockets.emit('worldUpdate', {
    'entities': this.game.entities.map(function (elem) {
      var r = {id: elem.id, type: elem.type, pos: elem.pos, rotation: elem.rotation, canPortal: elem.canPortal, grid: elem.grid}
      return r
    })
  })
}
...
GameServer.prototype.tickEvent = function () {
  ...
  this.game.entities.forEach(function (elem) {
    if (elem.changedSinceLastTick) {
      ...
      entitiesChangedSinceLastTick.push({id: elem.id, type: elem.type, pos: elem.pos, rotation: elem.rotation, canPortal: elem.canPortal, grid: elem.grid})
    }
  })
